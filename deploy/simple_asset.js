...
async function deploySmartContract(owner, name, symbol, network) {
    // Hard coded to compile and deploy the smart contract.
    const contractCode = await ethers.getContractFactory('Date');
    const imxAddress = getIMXAddress(network);
    const contractChain = await contractCode.deploy(owner, name, symbol, imxAddress);
  
    console.log('Deployed Contract Address:', contractChain.address);

    console.log('Verifying contract in 5 minutes...');
    await sleep(60000 * 5);
    await run("verify:verify", {
        address: contractChain.address,
